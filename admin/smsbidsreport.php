<?
	...
	if($_POST["submit"]!="" || $_GET["sdate"]!="")
	{
		...
		$qrysel = "select *,sum(bid_count) as totalbid from auction a left join products p on a.productID=p.productID left join bid_account ba on a.auctionId=ba.auction_id where  auc_start_date>='$startdate' and auc_end_date<='$enddate' and bidding_type='m' and bid_flag='d' group by auction_id";
		
		if($aid!="")
		{
		$qrysel = "select *,sum(bid_count) as totalbid from auction a left join products p on a.productID=p.productID left join bid_account ba on a.auctionId=ba.auction_id where a.auctionID='$aid' and auc_start_date>='$startdate' and auc_end_date<='$enddate' and bidding_type='m' and bid_flag='d' group by auction_id";
		}
...
<TABLE width="100%"  border=0 cellPadding=0 cellSpacing=10>
  <!--DWLayoutTable-->
    <TR> 
      <TD width="100%" class="H1">Relat&oacute;rio de lances por SMS</TD>
    </TR>
  	<TR>
		...
		?>		
		  ...
		  <?
			if($PageNo>1)
			{
			  $PrevPageNo = $PageNo-1;
		  ?>
		  <A class='paging' href="smsbidsreport.php?pgno=<?=$PrevPageNo;?>&<?=$urldata;?>">&lt; P&aacute;gina Anterior</A>
		  <?
		   }
		  ?> &nbsp;&nbsp;&nbsp;
		  <?php
			if($PageNo<$totalpage)
			{
			 $NextPageNo = 	$PageNo + 1;
		  ?>
		  <A class='paging' id='next' href="smsbidsreport.php?pgno=<?=$NextPageNo;?>&<?=$urldata;?>">Pr&amp;oacute;xima P&amp;aacute;gina &gt;</A>
		  <?
		   }
		  ?>
	<?
