...
<TABLE cellSpacing=10 cellPadding=0 width="100%" border=0>
  <TBODY>
  <TR>
      <TD class=H1>Administrar Conta</TD>
    </TR>
  <TR>
  ...
  for($i=0;$i<$MainLinksSize;$i++)
  {
	...
	?>
        <TR>
          ...
          <? if($ChildHREF=='#'){
			$usrpwd = base64_encode($_SESSION["usrname"]);
?>
          <TD vAlign=top height="13"><A class=menu_item href="defaultaccount.php" target="body" onClick="window.open('../helpdesk/admin.php?usrpwd=<?=$usrpwd?>');">
              <b><?php echo $ChildLinkTitle;?></b>
              </A>
		  </TD>
		<? }else{ ?>
		<TD vAlign=top height="13"><A 
            href="<? echo $ChildHREF; ?>" 
            target=body><B><? echo $LinkTitle; ?></B></A>
		  </TD>
		<? }?></TR>
        <TR>
