<?
	...
	if($_POST["submit"]!="" || $_GET["sdate"]!="")
	{
		...
		$qrysel = "select *,sum(actual_cost) as totalcost from auction a left join products p on a.productId=p.productID where auc_start_date>='$startdate' and auc_end_date<='$enddate' and auc_status='3' group by a.productID";
		if($pid!="")
		{
		$qrysel = "select *,sum(actual_cost) as totalcost from auction a left join products p on a.productId=p.productID where auc_start_date>='$startdate' and auc_end_date<='$enddate' and auc_status='3' and a.productID='$pid' group by a.productID";
		}
...
<TABLE width="100%"  border=0 cellPadding=0 cellSpacing=10>
  <!--DWLayoutTable-->
    <TR> 
      <TD width="100%" class="H1">Relat&oacute;rio de Lucro e Perca por item</TD>
    </TR>
  	<TR>
	...
	<tr>
		<td align="center" class="h2"><strong>Produto :</strong>
		<?php
			$qrypro = "select * from products";
		...
		<select name="product" style="width: 280px;">
			...
			<?
				...
				{
			?>
				<option <?=$pro->productID==$pid?"selected":"";?> value="<?=$pro->productID;?>"><?=$pro->name;?></option>
			<?
				}	
			?>
		</select>
	...
	?>
          <TABLE width="100%" border=1 cellSpacing=0 class=t-a>
            <!--DWLayoutTable-->
              <TR class=th-a> 
				...
				<TD width="7%" align="left" nowrap="nowrap">Produto ID</TD>
		...
		?>
				<TD width="7%" align="left" nowrap="nowrap"><?=$obj->productID;?></TD>
				<TD width="25%" align="center" nowrap="nowrap"><?=$obj->name;?></TD>
		...
		?>		
		  ...
		  <?
			if($PageNo>1)
			{
			  $PrevPageNo = $PageNo-1;
		  ?>
		  <A class='paging' href="itemfinincialreport.php?pgno=<?=$PrevPageNo;?>&<?=$urldata;?>">&lt; P&aacute;gina Anterior</A>
		  <?
		   }
		  ?> &nbsp;&nbsp;&nbsp;
		  <?php
			if($PageNo<$totalpage)
			{
			 $NextPageNo = 	$PageNo + 1;
		  ?>
		  <A class='paging' id='next' href="itemfinincialreport.php?pgno=<?=$NextPageNo;?>&<?=$urldata;?>">Pr&amp;oacute;xima P&amp;aacute;gina &gt;</A>
		  <?
		   }
		  ?>
	<?
