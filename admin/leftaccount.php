	...
	?>      
	 <DIV class="MenuHead" nowrap><font class="MenuHead"> 
      ...
      for($j=0;$j<$ChildLinksSize;$j++)
      {
		...
		?>
			<DIV class=mnpMenuRow  style="width: 188" align="left">&nbsp;<? if($ChildHREF!='#'){?>
			  <A class=menu_item href="<?php echo $ChildHREF ?>" target="body">
              <?php echo $ChildLinkTitle;?>
              </A>
			<? }else{ 
				$usrpwd = base64_encode($_SESSION['usrname']);
			?>
			<A class=menu_item href="defaultaccount.php" target="body" onClick="window.open('../helpdesk/admin.php?usrpwd=<?=$usrpwd?>');">
              <?php echo $ChildLinkTitle;?>
              </A>
			<? } ?></DIV>            <?php
        }
