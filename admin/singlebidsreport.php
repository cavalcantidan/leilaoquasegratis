<?
	...
	if($_POST["submit"]!="" || $_GET["sdate"]!="")
	{
		...
		$qrysel = "select *,sum(bid_count) as totalbid from auction a left join products p on a.productID=p.productID left join bid_account ba on a.auctionId=ba.auction_id where  auc_start_date>='$startdate' and auc_end_date<='$enddate' and bidding_type='s' and bid_flag='d' group by auction_id";
		
		if($aid!="")
		{
		$qrysel = "select *,sum(bid_count) as totalbid from auction a left join products p on a.productID=p.productID left join bid_account ba on a.auctionId=ba.auction_id where a.auctionID='$aid' and auc_start_date>='$startdate' and auc_end_date<='$enddate' and bidding_type='s' and bid_flag='d' group by auction_id";
		}
...
<TABLE width="100%"  border=0 cellPadding=0 cellSpacing=10>
  <!--DWLayoutTable-->
    <TR> 
      <TD width="100%" class="H1">Lances por cart&atilde;o de cr&eacute;dito</TD>
    </TR>
  	<TR>
          ...
          <TABLE width="100%" border=1 cellSpacing=0 class=t-a>
            <!--DWLayoutTable-->
              <TR class=th-a> 
				...
				<TD width="10%" nowrap="nowrap" align="center">Lances Simpes</TD>
