<?
	...
	if($_POST["submit"]!="" || $_GET["sdate"]!="")
	{
		...
		$qrysel = "select *,bd.id as bidid,DATE_FORMAT( bidpack_buy_date,  '%Y-%m-%d'  )  AS newdate,sum(ba.bid_count) as bidcounter
         from bid_account ba left join bidpack bd on ba.bidpack_id=bd.id
         where ba.bid_flag='c' and recharge_type='re' and bidpack_buy_date>='$startdate 00:00:01' and bidpack_buy_date<='$enddate 23:59:59'
         group by ba.bidpack_id order by bidcounter  desc";
...
<TABLE width="100%"  border=0 cellPadding=0 cellSpacing=10>
  <!--DWLayoutTable-->
    <TR> 
      <TD width="100%" class="H1">Relatorio de Valor de Pacotes</TD>
    </TR>
  	<TR>
	...
	?>
          <TABLE width="100%" border=1 cellSpacing=0 class=t-a>
            <!--DWLayoutTable-->
              <TR class=th-a> 
				...
				<TD width="10%" nowrap="nowrap" align="center">Valor Monet&aacute;rio</TD>
			 </TR>
		<?
		...
		?>
				...
				<TD align="right" nowrap="nowrap"><?=$Currency.number_format($total1*$obj->bidpack_price,2);?></TD>
			</tr>
		<?
			}
		?>		
