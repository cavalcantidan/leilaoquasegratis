<?
	...
	if($_POST["submit"]!="" || $_GET["sdate"]!="")
	{
		...
		$qrysel = "select *,sum(bid_count) as totalbid from auction a left join products p on a.productID=p.productID left join bid_account ba on a.auctionId=ba.auction_id where  auc_start_date>='$startdate' and auc_end_date<='$enddate' and bid_flag='d' group by auction_id";
		
		if($aid!="")
		{
		$qrysel = "select *,sum(bid_count) as totalbid from auction a left join products p on a.productID=p.productID left join bid_account ba on a.auctionId=ba.auction_id where a.auctionID='$aid' and auc_start_date>='$startdate' and auc_end_date<='$enddate' and bid_flag='d'  group by auction_id";
		}
...
<TABLE width="100%"  border=0 cellPadding=0 cellSpacing=10>
  <!--DWLayoutTable-->
    <TR> 
      <TD width="100%" class="H1">Lance dado por Leil&atilde;o</TD>
    </TR>
  	<TR>
	...
	?>
          <TABLE width="100%" border=1 cellSpacing=0 class=t-a>
            <!--DWLayoutTable-->
              <TR class=th-a> 
				...
				<TD width="7%" align="left" nowrap="nowrap">AuctionID</TD>
				<TD width="30%" align="center">Name</TD>
				<TD width="10%" nowrap="nowrap" align="center">Status</TD>
				<TD width="10%" nowrap="nowrap" align="center">Online Bids</TD>
		...
		<?
			while($obj = mysql_fetch_object($ressel))
			{
				...
				$qr = "select *,sum(bid_count) as totalsinglebid from bid_account where auction_id='".$obj->auctionID."' and bidding_type='s' and bid_flag='d' group by auction_id";
				$rs = mysql_query($qr);
				$objbids = mysql_fetch_object($rs);

				$qr1 = "select *,sum(bid_count) as totalbidbutlerbid from bid_account where auction_id='".$obj->auctionID."' and bidding_type='b' and bid_flag='d' group by auction_id";
				$rs1 = mysql_query($qr1);
				$objbids1 = mysql_fetch_object($rs1);

				$qr2 = "select *,sum(bid_count) as totalsmsbid from bid_account where auction_id='".$obj->auctionID."' and bidding_type='m' and bid_flag='d' group by auction_id";
				$rs2 = mysql_query($qr2);
		...
		?>		
		  ...
		  <?
			if($PageNo>1)
			{
			  $PrevPageNo = $PageNo-1;
		  ?>
		  <A class='paging' href="biddingreport.php?pgno=<?=$PrevPageNo;?>&<?=$urldata;?>">&lt; P&aacute;gina Anterior</A>
		  <?
		   }
		  ?> &nbsp;&nbsp;&nbsp;
		  <?php
			if($PageNo<$totalpage)
			{
			 $NextPageNo = 	$PageNo + 1;
		  ?>
		  <A class='paging' id='next' href="biddingreport.php?pgno=<?=$NextPageNo;?>&<?=$urldata;?>">Pr&amp;oacute;xima P&amp;aacute;gina &gt;</A>
		  <?
		   }
		  ?>
	<?
