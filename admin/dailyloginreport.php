...
<TABLE width="100%"  border=0 cellPadding=0 cellSpacing=10>
  <!--DWLayoutTable-->
    <TR> 
      <TD width="100%" class="H1">Relagt&oacute;rio Medio de Login/Logout </TD>
    </TR>
  	<TR>
	...
	?>
		...
		<?
			...
			$qr1 = "select * from login_logout where login_time>='$startdate 00:00:01' and logout_time<='$enddate 23:59:59' and user_id='".$obj->user_id."'";
			$rs1 = mysql_query($qr1);
			$total1 = mysql_num_rows($rs1);
		?>
				<TD width="7%" align="left" nowrap="nowrap"><?=$obj->user_id;?></TD>
				<TD align="left" nowrap="nowrap"><?=$obj->username;?></TD>
				<TD width="20%" nowrap="nowrap" align="center"><?=$total1;?></TD>
		...
		?>		
		  ...
		  <?
			if($PageNo>1)
			{
			  $PrevPageNo = $PageNo-1;
		  ?>
		  <A class='paging' href="dailyloginreport.php?pgno=<?=$PrevPageNo;?>&<?=$urldata;?>">&lt; P&aacute;gina Anterior</A>
		  <?
		   }
		  ?> &nbsp;&nbsp;&nbsp;
		  <?php
			if($PageNo<$totalpage)
			{
			 $NextPageNo = 	$PageNo + 1;
		  ?>
		  <A class='paging' id='next' href="dailyloginreport.php?pgno=<?=$NextPageNo;?>&<?=$urldata;?>">Pr&amp;oacute;xima P&amp;aacute;gina &gt;</A>
		  <?
		   }
		  ?>
	<?
