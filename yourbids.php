...
if(!isset($_SESSION["BPPENNYAUTOBID_LOGGED_IN"])) {
	...
	$sql = "SELECT
			...
			FROM BPPENNYAUTOBID_auctions a
			  INNER JOIN BPPENNYAUTOBID_bids b ON a.id = b.auction 
			  INNER JOIN BPPENNYAUTOBID_users c ON b.bidder = c.id          
			WHERE
			  b.bidder = ".intval($_SESSION['BPPENNYAUTOBID_LOGGED_IN'])." AND 
			  a.closed='0' AND
			  a.suspended=0 AND
	...
	$sql = "SELECT
			...
			SELECT
			  DISTINCT(a.id), a.title, a.starts
			FROM BPPENNYAUTOBID_auctions a
			  INNER JOIN BPPENNYAUTOBID_auctions_signed d ON a.id = d.auction_id 
			  INNER JOIN BPPENNYAUTOBID_users c ON d.user_id = c.id
			WHERE          
			  d.user_id = ".intval($_SESSION['BPPENNYAUTOBID_LOGGED_IN'])." AND
			  ((a.closed='0' AND a.starts <= ".$NOW.") OR (a.closed='1' AND a.second_starts <= ".$NOW.")) AND
			  a.suspended=0 AND 
			  a.auction_type = 2
			LIMIT 0, 100"; 
	...
	if($auction_id != ""){
		...
		$sql = "SELECT
			...
			WHERE
				b.bidder='".$_SESSION[BPPENNYAUTOBID_LOGGED_IN]."' AND
...
include_once phpa_include("template_yourbids_php.html");
